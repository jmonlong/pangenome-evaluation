...
```{r callsr}
if('callsr' %in% rownames(files.info)){
  vcf.l = lapply(colnames(files.info), function(exp){
    message(exp)
    vcf = readVcf(files.info['callsr', exp], param=ScanVcfParam(samples=NA))
    if(length(vcf)==0){
      sum.df = tibble(graph=exp, all=0, snv=0, indel.del=0, indel.ins=0,
                      sv.del=0, sv.ins=0)
